...
public class RequestValidator {
    ...
    public static ErrorResponse validateNotAuthorizedSession(HttpSession session) {
        if (session.getAttribute(session.getId()) != null) {
            return new ErrorResponse("User already authorized in this session", ExternalError.FORBIDDEN);
        }
        return null;
    }
    @Nullable
    public static ErrorResponse validateAlreadyAuthorizedSession(HttpSession session) {
        if (session.getAttribute(session.getId()) == null) {
            return new ErrorResponse("User not authorized in this session!", ExternalError.FORBIDDEN);
        }
        return null;
    }
